...
df_optimized.to_csv("game_logs_optimizired.csv")

#!!!!!!!
memory_stat_dataset(df_original, file_name_csv,  "memory_info_data_orijinal.json")
#!!!!!!!
memory_stat_dataset(df_optimized, "game_logs_optimizired.csv",  "memory_info_data_optimizired.json")
...
columns_name = ["date", "number_of_game", "day_of_week", "park_id",
            "v_manager_name", "length_minutes", "v_hits", "h_hits",
            "h_walks", "h_errors"]
...
#Формируем чанку(какой объем датасета(строк) будем держать в памяти)
